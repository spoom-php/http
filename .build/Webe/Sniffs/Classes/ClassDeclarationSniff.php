...
class Webe_Sniffs_Classes_ClassDeclarationSniff implements PHP_CodeSniffer_Sniff {
  ...
  public function process( PHP_CodeSniffer_File $phpcsFile, $stackPtr ) {
    ...
    // ellenőrzés, hogy biztos definícióban vagyunk
    ...
    // Nyitó zárójel elhelyezés vizsgálat
    ...
    // nincs semmi a zárójel után
    if( $tokens[ ( $curlyBrace + 1 ) ][ 'content' ] !== $phpcsFile->eolChar ) {
      $error = 'Opening %s brace must be the last symbol in the line';
    ...
    // 1 space van a zárójel előtt
    if( $tokens[ ( $curlyBrace - 1 ) ][ 'code' ] !== T_WHITESPACE || strlen( $tokens[ ( $curlyBrace - 1 ) ][ 'content' ] ) > 1 ) {

      $error = 'Expected exactly one space before opening brace';

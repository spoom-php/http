...
class Webe_Sniffs_ControlStructures_SwitchDeclarationSniff implements PHP_CodeSniffer_Sniff {
    ...
    while( ( $nextCase = $this->_findNextCase( $phpcsFile, ( $nextCase + 1 ), $switch[ 'scope_closer' ] ) ) !== false ) {
      ...
      if( $tokens[ $nextCode ][ 'code' ] !== T_CASE && $tokens[ $nextCode ][ 'code' ] !== T_DEFAULT ) {
        ...
        if( $nextCode !== false ) {
          ...
          if( $tokens[ $prevCode ][ 'code' ] !== T_COMMENT && $tokens[ $prevCode ][ 'code' ] !== T_DOC_COMMENT_CLOSE_TAG ) {
